...
def compute_audit_hash(entry_id: str, timestamp_value: str, sport: str, matchup: str, value_a: float, value_b: str, value_c: str) -> str:
    payload = f"{entry_id}|{timestamp_value}|{sport}|{matchup}|{value_a}|{value_b}|{value_c}"
...
def build_daily_file(prefix: str, date_str: str, batch: str = "001") -> Path:
    yyyy, mm, dd = date_str.split("-")
    return DAILY_DIR / f"{prefix}_{yyyy}_{mm}_{dd}_batch_{batch}.jsonl"
...
def auto_commit(paths: List[Path], message: str, push: bool = False) -> Dict[str, Any]:
    ...
    if push:
        ...
        result["pushed"] = True
        result["push_stdout"] = pushed.stdout.strip()
...
def log_bet(*, date_str: str, sport: str, matchup: str, mode: str, card_type: str, platform: str, market: str, legs: List[str], stake: float, payout_expected: float, sequence: int, boost_tag: str = "None", copy_id: str = "", edge_tags: Optional[List[str]] = None, model_tags: Optional[List[str]] = None, batch: str = "001") -> tuple[BetEntry, Path]:
    ensure_dirs()
    ...
    legs_raw = "; ".join(legs)
    audit_hash = compute_audit_hash(entry_id, timestamp_placed, sport, matchup, stake, platform, legs_raw)
    ...
    out_file = build_daily_file("placed_bets", date_str, batch)
    write_jsonl_line(out_file, asdict(entry))
    return entry, out_file


def log_settlement(*, entry_id: str, date_str: str, sport: str, matchup: str, result: str, legs_hit: int, legs_miss: int, stake: float, return_actual: float, final_score: str, settlement_notes: str, batch: str = "001") -> tuple[SettledEntry, Path]:
    ensure_dirs()
    timestamp_settled = now_iso()
    net = round(return_actual - stake, 2)
    roi = round(net / stake, 3) if stake else 0.0
    audit_hash = compute_audit_hash(entry_id, timestamp_settled, sport, matchup, return_actual, result, final_score)
    ...
    out_file = build_daily_file("settled_bets", date_str, batch)
    write_jsonl_line(out_file, asdict(entry))
    return entry, out_file


def log_postmortem(*, entry_id: str, date_str: str, sport: str, matchup: str, postmortem_type: str, result_type: str, hit_legs: List[str], missed_legs: List[str], environment_tags: List[str], variance_tags: List[str], model_notes: str, patch_required: bool, batch: str = "001") -> tuple[PostmortemEntry, Path]:
    ensure_dirs()
    ...
    out_file = build_daily_file("postmortems", date_str, batch)
    write_jsonl_line(out_file, asdict(entry))
    return entry, out_file


def main() -> None:
    parser = argparse.ArgumentParser(description="SharpEdge investor audit logger with auto git commit")
    parser.add_argument("--auto-commit", action="store_true", help="Auto-commit the updated JSONL file")
    parser.add_argument("--git-push", action="store_true", help="Push after commit (requires --auto-commit)")
    ...
    p_place.add_argument("--date", required=True)
    p_place.add_argument("--sport", required=True)
    p_place.add_argument("--matchup", required=True)
    ...
    if args.command == "place":
        ...
        default_message = f"Log placed bet {entry_obj.entry_id}"

    elif args.command == "settle":
        entry_obj, out_file = log_settlement(
        ...
        default_message = f"Log settled bet {entry_obj.entry_id}"

    else:
        entry_obj, out_file = log_postmortem(
            entry_id=args.entry_id,
            date_str=args.date,
            sport=args.sport,
            matchup=args.matchup,
        ...
        default_message = f"Log postmortem {entry_obj.entry_id}"

    output: Dict[str, Any] = {
        "entry": asdict(entry_obj),
        "file": str(out_file.relative_to(REPO_ROOT)),
    }

    if args.git_push and not args.auto_commit:
        raise SystemExit("--git-push requires --auto-commit")

    if args.auto_commit:
        commit_message = args.commit_message.strip() or default_message
        output["git"] = auto_commit([out_file], commit_message, push=args.git_push)
