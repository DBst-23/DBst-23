...
def prompt_scores(team_name: str, max_count: int = 4) -> List[int]:
    raw = input(f"Top scorer points for {team_name}, comma-separated [{max_count} slots]: ").strip()
    if not raw:
        return []
...
def build_inputs(team_a: TeamStats, team_b: TeamStats) -> Dict:
    game_name = input("\nGame label [Team A @ Team B]: ").strip() or f"{team_a.team_name} @ {team_b.team_name}"
    closing_spread = prompt_float("Closing spread (favorite negative)")
    halftime_live_spread = prompt_float("Halftime live spread")
    ...
    classifier_input = HalftimeEnvironmentInput(
        ...
        closing_total=closing_total,
        halftime_live_spread=halftime_live_spread,
        halftime_live_total=live_total,
        halftime_score_team1=team_a.points,
        halftime_score_team2=team_b.points,
        team1_stats=team1_ht,
        team2_stats=team2_ht,
    )
...
def main() -> None:
    print("=== SharpEdge LiveFlow Heat + Classifier Bridge ===")
    print("Enter current box-score inputs once. The runner will compute both Heat Index and Halftime Classifier outputs.")
    ...
    bundle = build_inputs(team_a, team_b)

    heat_result = compute_liveflow_heat_index(bundle["heat_input"])
    classifier_result = classify_environment(bundle["classifier_input"])

    payload = {
        "team_a": asdict(team_a),
        "team_b": asdict(team_b),
        "heat_input": asdict(bundle["heat_input"]),
        "classifier_input": asdict(bundle["classifier_input"]),
    ...
    print("\n=== SHARPEDGE DECISION SUMMARY ===")
    print(f"Heat Index: {heat_result['heat_index']} | {heat_result['classification']} | {heat_result['action_tag']}")
    print(f"Classifier: {classifier_result['environment_type']} | confidence={classifier_result['confidence']} | action={classifier_result['action_bias']}")

    if heat_result["action_tag"] == "UNDER_TRIGGER" and classifier_result["action_bias"] == "LEAN_UNDER":
        print("\nSharpEdge Read: Full alignment → strong live under environment.")
    elif heat_result["action_tag"] == "DO_NOT_FADE" or classifier_result["action_bias"] == "DO_NOT_AUTO_FADE":
        print("\nSharpEdge Read: Continuation environment → do not auto-fade.")
    else:
        print("\nSharpEdge Read: Mixed environment → monitor for cleaner trigger.")
